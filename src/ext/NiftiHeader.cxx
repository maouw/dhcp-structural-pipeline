...
namespace
{
    ...
    struct Scaling
    {
        long double mult, offset;
        Scaling(const long double& minval, const long double& maxval)
        {
            typedef std::numeric_limits<T> mylimits;
            long double mymin = mylimits::lowest();
            mult = (maxval - minval) / ((long double)mylimits::max() - mymin);//multiplying is the first step of decoding (after byteswap), so start with the range
